...
if uploaded_file:
    ...
    morph = cv2.morphologyEx(adaptive_thresh, cv2.MORPH_CLOSE, np.ones((2, 2), np.uint8))
    preprocessed_image = Image.fromarray(morph)
    with col2:
    ...
    with st.expander("📄 Raw OCR Output", expanded=False):
        ...
        data = pytesseract.image_to_data(preprocessed_image, lang='tam', config=custom_config, output_type=pytesseract.Output.DICT)
        extracted_text = " ".join(data['text'])
        confidences = [int(conf) for conf in data['conf'] if conf != '-1']
        average_conf = round(np.mean(confidences), 2) if confidences else 0
...
"""
    ...
    # Split the enhanced text into sentences
    sentences = re.split(r'(?<=[.!?]) +', enhanced_text)

    # Group sentences into chunks of four
    chunks = [' '.join(sentences[i:i + 4]) for i in range(0, len(sentences), 4)]

    # Display each chunk as a separate line
    for chunk in chunks:
        st.markdown(chunk)
...
st.markdown("""
    <div style='text-align: center; color: #7f8c8d; padding-top: 20px;'>
        Made with ❤️ by Tamil OCR Team | Powered by xAI
    </div>
""", unsafe_allow_html=True)
